function [ x, info ] = ELM_tcg_wls6_approxJ( res, rho, x0, pars )
...
% refresh = 1;
cg_iter = 0;
[ breakflag, exitflag ] = termination_test( f, gnorm, iter, 1, 1, pars );
while ~breakflag
    ...
    cg_iter = cg_iter + k;
    if info.iter == cg_maxit || strcmpi(info.exitflag, 'n')
        cg_tol = info.res;
    end
    ...
    if mod(iter,3) ~= 0 && ratio > p2 % norm(d) < norm(phi_g) % not a good choice when d is large
        refresh = 0;
    else 
...
%     fprintf(fid,'%4d \t %8.4e \t %8.4e \t %8.4e \t %8.4e \n',iter,norm(r),gnorm,norm(d),ratio);
    fprintf(fid,'%4d \t %8.4e \t %8.4e \t %8.4e \t %8.4e \t %d \t %4s \t %8.4e \t %s \t %d \n',...
                iter,f,gnorm,norm(d),ratio,refresh,flag,mu,info.exitflag,k);

    [ breakflag, exitflag ] = termination_test( f, gnorm, iter, ared, d, pars );
    if breakflag || toc(t0) > pars.walltime % || cg_iter > pars.cg_total_it
        break;
    end
end 
